...
async function syncDriverEntryToClientBatch(entryId, reviewedBy, visible) {
  ...
  const lotAmount = Number(full.lotAmount) || 0;
  ...
  const batchPayload = {
    ...
    wasteAmount,
    doublesAmount,
    lotQty: full.lotQty || 0,
    lotPrice: full.lotPrice || 0,
    lotAmount,
  ...
  try {
    ...
    for (const entry of batch.entries) {
      ...
      const payload = {
        ...
        lotQty: entry.lotQty || 0,
        lotPrice: entry.lotPrice || 0,
        lotAmount: calc.lotAmount,
