...
// GET/POST /api/admin/user-invite — public registration link (18h expiry)
...
// GET /api/admin/bookings/date-summary — grouped by date of going (logistics dashboard)
router.get('/bookings/date-summary', protect, adminOnly, async (req, res) => {
  try {
    ...
    console.log('[date-summary] Query:', JSON.stringify(q));
    ...
    console.log('[date-summary] Found bookings:', bookings.length);
    if (bookings.length > 0) {
      console.log('[date-summary] Sample booking:', JSON.stringify(bookings[0]));
    }
    ...
    for (const b of bookings) {
      const d = normalizeYmd(b.date) || b.date;
      if (!d) continue;
      if (!byDateMap.has(d)) {
        byDateMap.set(d, { date: d, users: [], totalWeightKg: 0 });
      }
    ...
    const byDate = [...byDateMap.values()]
      .map((g) => ({
        ...
        totalWeightKg: Math.round(g.totalWeightKg * 10) / 10
      }))
    ...
    const summary = {
      ...
      totalWeightKg: Math.round(
        byDate.reduce((s, g) => s + g.totalWeightKg, 0) * 10
      ) / 10
    };

    console.log('[date-summary] Summary:', summary);
    console.log('[date-summary] Grouped dates:', byDate.length);

    res.set('Cache-Control', 'no-store');
    res.json({ summary, byDate });
  } catch (error) {
    console.error('GET /admin/bookings/date-summary error:', error);
    res.status(500).json({ error: error.message });
  }
});
