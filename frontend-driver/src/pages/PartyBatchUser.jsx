...
export default function PartyBatchUser() {
      ...
      try {
        ...
        if (e) {
          setForm({
            ...
            goodSilkRatePerKg: e.goodSilkRatePerKg || rates.goodRate || '',
            wasteKg: e.wasteKg || '',
            wasteRatePerKg: e.wasteRatePerKg || rates.wasteRate || '',
            doublesKg: e.doublesKg || '',
            doublesRatePerKg: e.doublesRatePerKg || rates.doubleRate || ''
          });
        }
      } catch {
  ...
  const onSave = async () => {
    setSaving(true);
    try {
      await api.put(`/driver/party-batches/${batchId}/parties/${partyId}`, form);
      navigate(`/parties/${batchId}`);
    } catch (err) {
