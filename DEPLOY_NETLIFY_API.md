...
| `CORS_ORIGIN` | Your frontend Netlify URLs + localhost |
...
4. **Deploy** → copy site URL, e.g. `https://tracknow-backend.netlify.app`

5. Test: open `https://tracknow-backend.netlify.app/api/health` → should return JSON `{ status: "OK" }`

---

## 2. Point frontends to Netlify API

On **all** Netlify frontend sites → **Environment variables**:

| Key | Value |
|-----|--------|
| `VITE_API_URL` | `https://tracknow-backend.netlify.app/api` |

Use your real API hostname. **Trigger deploy** on client, admin, and driver.
